...
io.on("connection",(socket)=>{
    ...
    socket.on("joinRoom",({username,room})=>{
      ...
      // Welcome message 
      socket.emit("message",formateMessage("Mirror Mate","Welcome to Mirror Mate"));
      socket.emit("roomname",room)
      // Broadcasting other users
      socket.broadcast.to(user.room).emit("message",formateMessage("Mirror Mate",`${username} has joined the chat`));
...
console.log(msg)
    ...
    const user = userLeave(socket.id);
       ...
       io.to(user.room).emit("message",formateMessage("Mirror Mate",`${user.username} has left the chat`));
